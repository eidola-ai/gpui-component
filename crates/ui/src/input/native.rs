...
mod macos {
    ...
    fn ns_view(window: &Window) -> Option<&AnyObject> {
        ...
        // unimplemented!() — a panic, not an Err, so `.ok()?` alone cannot
        // skip it. Catch the unwind so a window without a real platform
        // backing simply opts out. Removal trigger: TestWindow returning
